...
export function createRuntimes(profiles: readonly AgentProfile[]): Map<string, AgentRuntime> {
  ...
  if (profiles.some((profile) => profile.provider === "codex")) runtimes.set("codex", new CodexRuntime());
  for (const profile of profiles) {
    if (!runtimes.has(profile.provider)) throw new Error(`No runtime adapter is available for provider: ${profile.provider}`);
  }
  return runtimes;
}
