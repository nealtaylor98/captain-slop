...
export type AgentStatus =
  "queued" | "running" | "waiting-for-approval" | "completed" | "failed" | "cancelled";
...
export interface PolicyProposal extends ApprovalRequest {
...
export class PolicyManager {
  ...
  constructor(globalTools: ToolName[]) {
    for (const tool of globalTools) this.global.add(tool);
  }
  globalTools(): ReadonlySet<ToolName> {
    return this.global;
  }
  propose(
    requestedBy: string,
    scope: PolicyProposal["scope"],
    tool: ToolName,
    reason: string,
  ): PolicyProposal {
...
export function effectiveTools(
  baseline: ToolName[],
  profileGrants: ToolName[],
  approvedTaskGrants: ToolName[],
): Set<ToolName> {
  return new Set([...baseline, ...profileGrants, ...approvedTaskGrants]);
}

export function requestPolicyChange(
  requestedBy: string,
  tool: ToolName,
  reason: string,
): ApprovalRequest {
  return {
    id: `approval-${requestedBy}-${tool}-${Date.now()}`,
    requestedBy,
    tool,
    reason,
    status: "pending",
