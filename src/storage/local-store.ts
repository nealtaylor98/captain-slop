...
export class LocalStore implements Persistence {
  ...
  async appendEvent(sessionId: string, event: StoredEvent): Promise<void> {
    (this.state.events[sessionId] ??= []).push(event);
    await this.persist();
  }
  async compactSession(sessionId: string, summary: string, compactedAt: number): Promise<void> {
    const session = this.state.sessions.find((candidate) => candidate.id === sessionId);
    if (!session) throw new Error(`Unknown session: ${sessionId}`);
    session.compaction = { compactedAt, summary };
    session.updatedAt = Math.max(session.updatedAt, compactedAt);
    this.state.events[sessionId] = [];
    await this.persist();
  }
  async saveReport(report: WorkerReport): Promise<void> {
    this.state.reports.push(report);
    await this.persist();
  }
  async cleanup(now: number): Promise<number> {
    const cutoff = now - 30 * 24 * 60 * 60 * 1000;
    const expired = this.state.sessions
