...
test("main controller stores only main-agent exchanges", async () => {
  const events: string[] = [];
  const controller = new MainAgentController(new FakeRuntime({ hi: [{ type: "text", text: "hello" }, { type: "completed", summary: "done" }] }), { id: "main", provider: "fake", model: "demo", instructions: "", allowedTools: [] }, (event) => events.push(event.type));
