...
const profile = { id: "researcher", provider: "fake", model: "demo", instructions: "report", allowedTools: ["read_file"] };
...
test("scheduler runs multiple instances of one profile and routes reports", async () => {
  ...
  const scheduler = new WorkerScheduler({ runtimes: new Map([["fake", runtime]]), profiles: [profile], globalTools: [], concurrency: 2 });
  ...
  assert.deepEqual(scheduler.reports().map((report) => report.summary).sort(), ["first report", "second report"]);
...
test("scheduler delivers a completed worker report to the main-agent route", async () => {
  ...
  const scheduler = new WorkerScheduler({ runtimes: new Map([["fake", runtime]]), profiles: [profile], globalTools: [], concurrency: 1, onReport: (report) => routed.push(report.summary) });
...
test("scheduler retains queued work at its concurrency limit and can cancel it", async () => {
  ...
  const scheduler = new WorkerScheduler({ runtimes: new Map([["fake", runtime]]), profiles: [profile], globalTools: [], concurrency: 1 });
...
test("scheduler retains a runtime session id discovered during the first streamed turn", async () => {
  class NativeIdRuntime implements AgentRuntime {
    capabilities() { return { enforcedTools: false, limitations: [] }; }
    async createSession(_options: SessionOptions): Promise<RuntimeSession> { return { id: "pending", agentId: "worker" }; }
    async resumeSession(id: string): Promise<RuntimeSession> { return { id, agentId: "worker" }; }
    async *send(session: RuntimeSession) { session.id = "native-session"; yield { type: "completed" as const, summary: "done" }; }
    async cancel(_session: RuntimeSession): Promise<void> {}
  }
  const scheduler = new WorkerScheduler({ runtimes: new Map([["fake", new NativeIdRuntime()]]), profiles: [profile], globalTools: [], concurrency: 1 });
