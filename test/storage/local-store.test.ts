...
test("local store persists sessions, events and reports", async () => {
  ...
  try {
    ...
    await store.appendEvent("s1", { at: 2, agentId: "main", event: { type: "text", text: "hello" } });
    await store.saveReport({ workerId: "w1", status: "completed", summary: "done", artifacts: [], changes: [] });
    const restored = new LocalStore(join(dir, "state.json")); await restored.open();
...
test("retention removes sessions and their event streams older than thirty days", async () => {
  const dir = await mkdtemp(join(tmpdir(), "tcode-"));
  try {
    const store = new LocalStore(join(dir, "state.json")); await store.open();
    const old = 0; const recent = 31 * 24 * 60 * 60 * 1000;
    await store.saveSession({ id: "old", createdAt: old, updatedAt: old, mainAgentId: "m" });
    await store.saveSession({ id: "recent", createdAt: recent, updatedAt: recent, mainAgentId: "m" });
    await store.cleanup(31 * 24 * 60 * 60 * 1000);
    assert.deepEqual(store.sessions().map((session) => session.id), ["recent"]);
  } finally { await rm(dir, { recursive: true, force: true }); }
});
...
test("concurrent event appends are serialized without competing temporary-file renames", async () => {
  ...
  try {
    ...
    await Promise.all(Array.from({ length: 8 }, (_, at) => store.appendEvent("s", { at, agentId: "m", event: { type: "text", text: String(at) } })));
